...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="lg:w-80 flex-shrink-0">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 border border-gray-100 dark:border-gray-700 sticky top-24">
                ...
                <form method="GET" action="{{ route('cocineros') }}" id="filtrosCocineros">
                    <!-- Búsqueda -->
                    <div class="mb-6">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Buscar cocinero</label>
                        <div class="relative">
                    ...
                    <!-- Calificación mínima -->
                    <div class="mb-6">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Calificación mínima</label>
                        <div class="space-y-2">
                            @foreach([4.5, 4.0, 3.5, 3.0] as $rating)
                                <label class="flex items-center cursor-pointer group">
                                    <input type="radio" name="calificacion_min" value="{{ $rating }}"
                                           ...
                                           class="w-4 h-4 text-yellow-500 border-gray-300 dark:border-gray-600 focus:ring-yellow-500">
                                    <span class="ml-3 text-sm text-gray-700 dark:text-gray-200 group-hover:text-gray-900 dark:text-white flex items-center">
                            ...
                            @if(request('calificacion_min'))
                                <label class="flex items-center cursor-pointer group">
                                    <input type="radio" name="calificacion_min" value=""
                                           class="w-4 h-4 text-gray-500 dark:text-gray-400 border-gray-300 dark:border-gray-600 focus:ring-gray-500">
                                    <span class="ml-3 text-sm text-gray-500 dark:text-gray-400 group-hover:text-gray-700 dark:text-gray-200">
                    ...
                    @if($especialidades->count() > 0)
                        <div class="mb-6">
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Especialidad</label>
                            <select name="especialidad" class="w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors bg-white dark:bg-gray-800 text-sm">
                    ...
                    <!-- Radio de entrega -->
                    <div class="mb-6">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Radio de entrega mínimo</label>
                        <select name="radio_min" class="w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors bg-white dark:bg-gray-800 text-sm">
                    ...
                    <!-- Ordenar por -->
                    <div class="mb-6">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Ordenar por</label>
                        <select name="ordenar" class="w-full px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors bg-white dark:bg-gray-800 text-sm">
        ...
        <div class="flex-1">
            <!-- Barra de resultados -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 mb-6 border border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="text-gray-600 dark:text-gray-400">
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $cocineros->total() }}</span> cocinero(s) encontrado(s)
                    @if(request('buscar'))
            ...
            <div class="lg:hidden mb-6">
                <button type="button" onclick="document.getElementById('mobile-filters-cocineros').classList.toggle('hidden')"
                        class="w-full py-3 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 rounded-lg font-medium hover:bg-gray-50 dark:bg-gray-700 transition-colors flex items-center justify-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.414A1 1 0 013 6.586V4z"/>
                    </svg>
                    Mostrar Filtros
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                @forelse($cocineros as $cocinero)
                    <div class="group bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700">
                        <!-- Header del cocinero -->
                        <div class="relative h-32 bg-gradient-to-br from-primary-500 to-orange-400">
                            ...
                            @if($cocinero->esta_disponible)
                                <div class="absolute top-4 right-4">
                                    <span class="bg-green-500 text-white text-xs font-medium px-3 py-1 rounded-full flex items-center">
                                        <span class="w-2 h-2 bg-white dark:bg-gray-800 rounded-full mr-2 animate-pulse"></span>
                                        Disponible
                                    </span>
                                </div>
                            @endif
                            @if($cocinero->radio_entrega_km)
                                <div class="absolute top-4 left-4">
                                    <span class="bg-white/90 backdrop-blur-sm text-gray-700 dark:text-gray-200 text-xs font-medium px-2 py-1 rounded-full flex items-center">
                        ...
                        <div class="pt-16 p-6">
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <h3 class="font-bold text-gray-900 dark:text-white text-xl group-hover:text-primary-600 transition-colors">
                                    ...
                                    <div class="flex items-center mt-2">
                                        <div class="flex items-center bg-yellow-50 dark:bg-yellow-900/30 px-3 py-1 rounded-full">
                                            ...
                                            <span class="ml-1 font-bold text-gray-900 dark:text-white">{{ number_format($cocinero->calificacion_promedio, 1) }}</span>
                                        </div>
                                        <span class="text-gray-500 dark:text-gray-400 text-sm ml-2">({{ $cocinero->total_pedidos }} pedidos)</span>
                                    </div>
                                </div>
                            </div>

                            <p class="text-gray-600 dark:text-gray-400 mb-5 line-clamp-3 leading-relaxed text-sm">
                            ...
                            @if($cocinero->especialidades && count($cocinero->especialidades) > 0)
                                <div class="flex flex-wrap gap-2 mb-5">
                                    ...
                                    @if(count($cocinero->especialidades) > 3)
                                        <span class="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 px-3 py-1 rounded-full font-medium">
                            ...
                            <div class="grid grid-cols-3 gap-2 mb-5">
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400">Años exp.</div>
                                </div>
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center">
                            ...
                            @if($cocinero->direccion)
                                <div class="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-5">
                ...
                @empty
                    <div class="col-span-full text-center py-20">
                        ...
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">No hay cocineros disponibles</h3>
                        <p class="text-gray-500 dark:text-gray-400 mb-6">Intenta con otros filtros de búsqueda</p>
...
<div id="mobile-filters-cocineros" class="hidden lg:hidden fixed inset-0 bg-black/50 z-50" onclick="this.classList.add('hidden')">
    <div class="absolute right-0 top-0 h-full w-full max-w-sm bg-white dark:bg-gray-800 overflow-y-auto" onclick="event.stopPropagation()">
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-lg font-bold text-gray-900 dark:text-white">Filtros</h2>
                <button type="button" onclick="document.getElementById('mobile-filters-cocineros').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 dark:text-gray-400">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
            </div>
            <p class="text-sm text-gray-600 dark:text-gray-400">Utiliza el panel de filtros en la parte superior para filtrar los cocineros.</p>
        </div>
    </div>
</div>
@endsection
