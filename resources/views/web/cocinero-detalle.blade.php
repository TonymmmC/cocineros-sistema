...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <nav class="mb-8">
        <ol class="flex items-center space-x-2 text-sm">
            ...
            <li>
                <a href="{{ route('cocineros') }}" class="text-gray-500 hover:text-primary-600 transition-colors">Cocineros</a>
            </li>
            <li>
                <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            ...
            <li>
                <span class="text-gray-900 font-medium">{{ $cocinero->nombre_completo }}</span>
            </li>
        </ol>
    </nav>

    <!-- Perfil del Cocinero -->
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100 mb-12">
        ...
        <div class="relative px-8 pb-8">
            ...
            <div class="absolute -top-16 left-8">
                <div class="w-32 h-32 bg-white rounded-full p-1 shadow-xl">
                    <div class="w-full h-full bg-gradient-to-br from-primary-100 to-primary-200 rounded-full flex items-center justify-center text-5xl overflow-hidden">
            ...
            <div class="pt-20">
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-6">
                    <div>
                        <h1 class="text-3xl lg:text-4xl font-bold text-gray-900 mb-3">{{ $cocinero->nombre_completo }}</h1>
                        <div class="flex items-center gap-4">
                            <div class="flex items-center bg-yellow-50 px-4 py-2 rounded-full">
                                ...
                                <span class="ml-2 text-2xl font-bold text-gray-900">{{ number_format($cocinero->calificacion_promedio, 1) }}</span>
                            </div>
                            <span class="text-gray-500">({{ $cocinero->total_pedidos }} pedidos completados)</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="flex flex-wrap gap-3">
                            @foreach($cocinero->especialidades as $especialidad)
                                <span class="bg-primary-50 text-primary-700 px-4 py-2 rounded-full font-medium">
                                    {{ $especialidad }}
                                </span>
                            @endforeach
                        </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-8">
                    <div class="bg-gradient-to-br from-primary-50 to-white rounded-xl p-6 text-center border border-primary-100">
                        <div class="text-3xl font-bold text-primary-600 mb-1">{{ $cocinero->productos_count }}</div>
                        <div class="text-sm text-gray-600 font-medium">Productos</div>
                    </div>
                    <div class="bg-gradient-to-br from-primary-50 to-white rounded-xl p-6 text-center border border-primary-100">
                        <div class="text-3xl font-bold text-primary-600 mb-1">{{ $cocinero->total_pedidos }}</div>
                        <div class="text-sm text-gray-600 font-medium">Pedidos</div>
                    </div>
                    <div class="bg-gradient-to-br from-primary-50 to-white rounded-xl p-6 text-center border border-primary-100">
                        <div class="text-3xl font-bold text-primary-600 mb-1">{{ $cocinero->experiencia_anios ?? 0 }}</div>
                        <div class="text-sm text-gray-600 font-medium">Años de exp.</div>
                    </div>
                    <div class="bg-gradient-to-br from-primary-50 to-white rounded-xl p-6 text-center border border-primary-100">
                        <div class="text-3xl font-bold {{ $cocinero->esta_disponible ? 'text-green-600' : 'text-red-600' }} mb-1">
    ...
    <div>
        <div class="flex justify-between items-end mb-8">
            <div>
                ...
                <p class="text-gray-600">{{ $productos->total() }} producto(s) disponible(s)</p>
            </div>
        </div>
        ...
        @else
            <div class="text-center py-20 bg-white rounded-2xl border border-gray-100">
                <div class="text-6xl mb-4 opacity-30"><svg class='w-16 h-16 text-gray-300' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z'/></svg></div>
                <h3 class="text-xl font-semibold text-gray-900 mb-2">Sin productos disponibles</h3>
                <p class="text-gray-500">Este cocinero aún no tiene productos publicados</p>
            </div>
        @endif
    </div>
</div>
